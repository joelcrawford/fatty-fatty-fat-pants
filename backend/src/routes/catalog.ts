...
export function createCatalogRouter(db: Db): Router {
  ...
  const calPerMin = (met: number, kg: number) => Math.round((met * 3.5 * kg) / 200 * 10) / 10;
  ...
  router.get("/exercises", (_req: Request, res: Response) => {
    try {
      ...
      res.json({
        success: true,
        data: rows.map((e) => ({ ...e, cal_per_min: calPerMin(e.met, referenceKg), cal_per_min_weight_kg: referenceKg })),
      });
    } catch (err) {
