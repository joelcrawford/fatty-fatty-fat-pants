...
//
// Framework-free: no React, no DOM, no Node. The web app constructs it with
// localStorage and a Web Lock; the mobile app with expo-secure-store and no
...
import type { User, ValidationDetail, AuthSession } from "./types";
...
export class ApiError extends Error {
  constructor(message: string, readonly status: number, readonly details?: ValidationDetail[]) {
    super(message);
    this.name = "ApiError";
  }
}
...
export class Session {
  ...
  private async startSession(data: AuthSession): Promise<User> {
