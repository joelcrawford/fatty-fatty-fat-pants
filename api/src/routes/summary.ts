...
import { Db } from "../db";
import { netCarbs, round } from "@nutrition/shared";
...
export function createSummaryRouter(db: Db): Router {
  ...
  router.get("/day/:date", validate({ params: dateParams }), (req: Request, res: Response) => {
    try {
      ...
      const net_carbs = netCarbs(food.total_carbs, food.total_fiber);
      const net_cal = food.total_cal - exercise.exercise_cal;

      const summary: DaySummary = {
        date,
        total_cal: Math.round(food.total_cal),
        total_protein: round(food.total_protein),
        total_carbs: round(food.total_carbs),
        total_fat: round(food.total_fat),
        total_fiber: round(food.total_fiber),
  ...
  router.get("/range", validate({ query: rangeQuery }), (req: Request, res: Response) => {
    try {
      ...
      const summaries: DaySummary[] = rows.map(r => ({
        ...
        total_protein: round(r.total_protein),
        total_carbs: round(r.total_carbs),
        total_fat: round(r.total_fat),
        total_fiber: round(r.total_fiber),
        net_carbs: round(netCarbs(r.total_carbs, r.total_fiber)),
